...
public class DatePicker extends StackPane {
        ...
        private ChangeListener<Date> datechanged = new ChangeListener<Date>() {
            @Override public void changed(ObservableValue<? extends Date> o, Date oldVal, Date newVal){
                if(newVal!=null) {
                    dateProperty.setValue(newVal);
                    Calendar newdate = Calendar.getInstance();
                    newdate.setTimeInMillis(newVal.getTime());
                    month = newdate.get(Calendar.MONTH);
                    year = newdate.get(Calendar.YEAR);
                    monthYear.setText(getMonthYear(newVal));
                    setDayCells();
                }
            }
        };
	...
	private ObjectProperty<Date> dateProperty = new SimpleObjectProperty<Date>(new Date((new GregorianCalendar()).getTimeInMillis()));
	...
	private String[] cellStyleList = {DATEPICKER_OTHERMONTH, DATEPICKER_TODAY, DATEPICKER_MONTH};
        ...
        private final Label monthYear = new Label(getMonthYear(dateProperty.get()));
	...
	public DatePicker(){
		...
		year = today.get(Calendar.YEAR);
                dateProperty.addListener(datechanged);
		...
		decrementArrow.setOnMouseClicked(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent me) {
				...
				monthYear.setText(getMonthYear(month, year));
				setDayCells();
				me.consume();
			}
		});
		...
		todayButton.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent ae) {
				dateProperty.setValue(new Date((new GregorianCalendar(0, 0, 0)).getTimeInMillis()));
				dateProperty.setValue(
						new Date((new GregorianCalendar(
								today.get(Calendar.YEAR), 
								today.get(Calendar.MONTH), 
								today.get(Calendar.DAY_OF_MONTH))).getTimeInMillis()));
	...
	private boolean isToday(Calendar calendar){
        ...
        private boolean isSelectedday(Calendar calendar) {
            if(this.dateProperty.get()!=null) {
                Calendar selection = Calendar.getInstance();
                selection.setTime(this.dateProperty.get());
                return selection.get(Calendar.YEAR) == calendar.get(Calendar.YEAR) && 
                                selection.get(Calendar.MONTH) == calendar.get(Calendar.MONTH) &&
                                selection.get(Calendar.DAY_OF_MONTH) == calendar.get(Calendar.DAY_OF_MONTH);
            } else {
                return false;
            }
        }
